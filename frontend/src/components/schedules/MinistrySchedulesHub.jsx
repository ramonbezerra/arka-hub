        ...
        <section>
            <div className="lg:w-[88%] sm:w-[88%] w-full mx-auto shadow-2xl p-4 rounded-xl h-fit self-center bg-gray-100">
                <div className="items-center text-gray-600 p-4 flex justify-between">
                    <h1 className="lg:text-3xl md:text-2xl text-xl">{t('Ministry schedules')}</h1>
                </div>
                {loading && <div className="text-gray-600 mb-2">{t('Loading ministries...')}</div>}
                {error && <div className="text-red-500 mb-2">{error}</div>}
                {!loading && !error && ministries.length === 0 && (
                    <div className="text-gray-600 mb-2">{t('You are not assigned to any ministry.')}</div>
                )}
                {!loading && !error && ministries.length > 0 && (
                    <div className="mb-4 overflow-x-auto">
                        <table className="w-full table-fixed divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                <tr><th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('Ministry')}</th>
